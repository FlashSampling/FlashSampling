...
- Every CUTLASS plan gate must leave a human-verifiable packet under `benchmarking/modal-results/cutlass-<gate-name>/`: a structured summary with expected and actual results, per-case expected/actual/error/pass data, the complete log, and a finding that states failure signatures and limitations. Performance and distribution gates must also retain their raw measurements and statistics. A successful process exit alone does not complete a gate.
