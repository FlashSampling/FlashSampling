...
- Before the next CUTLASS register-pressure experiment, establish a dtype-matched ordinary CUTLASS GEMM that remains within 5% of cuBLAS for every primary shape and H=1 through H=256. The current diagnostic CUTLASS GEMM plus argmax is slower than cuBLAS plus argmax in all 36 configurations, with median ratios of 1.17 on H100 and 1.27 on B200, but the comparison mixes FP32 CUTLASS output with BF16 cuBLAS output. Tune the ordinary GEMM first and stop the port if it cannot pass. Only then continue epilogue work, keeping variants diagnostic until Gate 1a ownership and the dependent Gate 1b-1g tests pass. The complete takeover sequence is in `findings/cutlass/15-greedy-profile-stage2.md`.
- **Immediate CUTLASS next action:** Add one diagnostic gate comparing ordinary CUTLASS GEMM directly with cuBLAS GEMM using identical BF16 inputs, BF16 output, dimensions, padding, and cold-L2 timing across both primary shapes and H=1 through H=256. Do not change the fused epilogue until this comparison is complete.
