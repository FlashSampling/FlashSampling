...
- CUTLASS handoff: continue from the numbered `Handoff` section in `findings/cutlass/16-ordinary-gemm-specialization.md`. The immediate task is a persistent multi-variant ordinary-GEMM tuning runner, starting with legal 64x128x64, 128x64x64, and 128x128x64 tiles and architecture-specific schedules. Preserve all candidate rows, dispatch by architecture and H, and do not modify the fused epilogue or begin Philox until the ordinary `CUTLASS/cuBLAS <= 1.05` prerequisite passes everywhere.
