...
- Gate 2d's first 2-SM fused-EVT experiment passes on B200 with `make modal-cutlass GATE=winning-schedule-evt`. Direct compact per-tile output does not merge cooperating CTAs, so the gate uses a packed 64-bit atomic final reduction. The initial equal-value tie failure came from double-counting cluster rank: CUTLASS's 2-SM `tile_coord_mnkl.m` is already a per-CTA 64-row coordinate. Use `global_m = cta_m * 64 + consumer_thread % 64`, not a 128-row cluster offset plus rank. All 400 exact comparisons and both sanitizers pass; the shared Gate 1g source still passes all 1,744 H100/B200 comparisons and sanitizers. See `findings/cutlass/21-winning-schedule-evt.md`.
