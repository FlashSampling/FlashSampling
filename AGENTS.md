...
- The first Gate 2d production transplant dispatches B200 H<=64 to the winning `128x64x128` 2-SM cluster-(2,1,1) schedule with the 64-bit atomic final reduction. This path emits one final candidate per hidden state and skips the separate Stage 2 merge. The complete H100/B200 Gate 2a matrix still passes. H=128 and H=256 remain on the old provider until their winning donors are transplanted.
...
- Gate 2c passed on B200. Gate 2d accumulator ownership passed for all five winning 2-SM GEMM schedule instantiations with `make modal-cutlass GATE=winning-schedule-layout`. The 128x64, 256x128, and 256x256 output tiles require three distinct coordinate formulas; see `findings/cutlass/20-winning-schedule-accumulator-layout.md`. The H<=64 fused epilogue is now in the production provider. H=128 and H=256 donor transplants are next.
