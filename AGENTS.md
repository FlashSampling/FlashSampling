...
- `b200-warp-spec-single-tile-crash.md` — On B200 / Triton 3.6, calling the FMMS Triton kernel with `num_tiles=1` + `warp_specialize=True` crashes the `NVWSInsertAref` MLIR pass (`PassManager::run failed`). Triggered by the chi-squared TP2 tests' tiny synthetic inputs (V=100, H=1). Fix: gate `WARP_SPECIALIZE` on `V > 2 * MIN_BLOCK_SIZE_V` (=256) in `core.py` so prod V (>=128k) keeps WS on while test inputs skip it. `make modal-speed-test` is unaffected.
