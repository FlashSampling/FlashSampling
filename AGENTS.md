...
- `findings/cutlass/14-greedy-performance.md` — Gate 2b reached a no-go decision under the predeclared 5% threshold. Across both primary shapes and H=1,2,4,...,256, only 12/36 configurations passed. Median CUTLASS-FMMS-to-CUTLASS-GEMM-plus-argmax ratios were 1.14 on H100 and 1.20 on B200; the worst was 1.38 at V=151,936, D=4,096, H=1 on B200. CUTLASS FMMS was slower than Triton FMMS in 35/36 configurations and cuBLAS plus argmax in 36/36. Do not begin Gate 3 on the current implementation. Profile representative points only to guide a rework decision, then rerun Gate 2b, or stop the CUTLASS port.
