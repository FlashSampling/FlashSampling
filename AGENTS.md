...
- The complete B200 H=1 through H=256 production dispatch now uses the Gate 2c fused-EVT donors with packed 64-bit atomic final reduction. H<=64 uses `128x64x128` cluster (2,1,1), H=128 uses `256x128x64` cluster (4,1,1) for D=4,096 and `256x128x128` cluster (2,1,1) for D=8,192, and H=256 uses `256x256x64` cluster (2,1,1). `make modal-cutlass GATE=greedy-provider` passed all 52 cases plus all 18 shared pytest cases on both H100 and B200. Gate 2b performance timing is next.
