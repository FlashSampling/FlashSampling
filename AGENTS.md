...
- Gate 2b cannot remove D with only `ElementD=void` and the standard TMA collective on both target architectures. CUTLASS 4.6.1 accepts this path on SM90 after the EVT advertises `ElementAux`, but the SM100 collective still instantiates void D shared-memory sizing and a void TMA descriptor; current CUTLASS `main` has the same implementation. The next implementation is an adapted or handwritten SM100 epilogue collective that keeps the CUTLASS mainloop and validated candidate reduction while omitting the D shared-memory pipeline, descriptor, and global store. Do not substitute aliased scratch stores because that retains traffic and introduces races.
