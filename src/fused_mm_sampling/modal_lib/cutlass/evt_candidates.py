...
OUTPUT_DIR = result_dir("07-evt-candidates")
...
def record_sm90() -> dict[str, str]:
    return run_compute_sanitizer(
        "/opt/fmms/cutlass_evt_candidates_sm90",
        ("memcheck", "racecheck"),
        CSV_HEADER,
    )
...
def record_sm100() -> dict[str, str]:
    return run_compute_sanitizer(
        "/opt/fmms/cutlass_evt_candidates_sm100",
        ("memcheck", "racecheck"),
        CSV_HEADER,
    )
...
def _validate(csv_text: str, architecture: str) -> pd.DataFrame:
    ...
    actual_rows = (
        cases.groupby("case", as_index=False).agg(actual=("column", "count"))
    )
...
def main() -> None:
    ...
    case_summary = (
        cases.assign(
            value_match=cases["expected_value_bits"].eq(cases["actual_value_bits"]),
